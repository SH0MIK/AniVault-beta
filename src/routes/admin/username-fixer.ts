...
import { generateFriendlyUsernameBase, isAutoGeneratedUsername } from '../../lib/auth';
...
adminUsernameFixerRoutes.on(['GET', 'POST'], '/admin/username_fixer.php', async (c) => {
  ...
  html += `
<div class="admin-header"><h1>🔧 Username Fixer</h1><span class="text-muted">Scan and fix invalid or auto-generated usernames</span></div>
${err ? `<div class="alert alert-error mb-2">⚠️ ${h(err)}</div>` : ''}
${suc ? `<div class="alert alert-success mb-2">✅ ${h(suc)}</div>` : ''}

${flagged.length === 0 ? `
<div class="card card-body text-center" style="padding:3rem;"><div style="font-size:3rem;margin-bottom:1rem;">✅</div><h2>All usernames look clean!</h2><p class="text-muted">No accounts with invalid characters or untouched auto-generated names were found.</p></div>` : `
<div class="alert alert-error mb-3" style="display:flex;align-items:center;gap:1rem;justify-content:space-between;flex-wrap:wrap;">
  <span>⚠️ Found <strong>${flagged.length}</strong> account(s): <strong>${badUsers.length}</strong> with invalid usernames, <strong>${guestUsers.length}</strong> still on an auto-generated guest name.</span>
  ${isOwner ? `<form method="POST" onsubmit="return confirm('Auto-fix all ${flagged.length} usernames? Invalid ones get cleaned up, auto-generated guest names get a fresh random handle.')"><input type="hidden" name="action" value="auto_fix_all"><button type="submit" class="btn btn-primary btn-sm">⚡ Auto-Fix All</button></form>` : ''}
</div>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>${u.reason === 'invalid'
          ...
          : `<span class="badge" style="background:rgba(255,193,7,0.15);color:#ffc107;">Auto-generated</span>`}</td>
