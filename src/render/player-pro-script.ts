...
function applyFsSize(){
  if(!root.classList.contains('sp-fs')){ root.style.width=''; root.style.height=''; return; }
...
window.addEventListener('resize',applyFsSize);
window.addEventListener('orientationchange',()=>setTimeout(applyFsSize,300));
if(window.visualViewport) window.visualViewport.addEventListener('resize',applyFsSize);

function syncFsIcon(){
  const isFs=document.fullscreenElement===root||document.webkitFullscreenElement===root;
  root.classList.toggle('sp-fs',isFs);
  applyFsSize();
