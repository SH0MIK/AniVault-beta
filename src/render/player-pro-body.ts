// Professional custom-controls player (replaces the native <video controls>
// "default player" in player-body.ts). Full feature set: custom seek bar
// with buffered range + hover preview, volume slider, quality menu, speed
// menu, subtitle menu (track picker + size/position/font/color/background
// customization), keyboard shortcuts, double-tap seek zones on mobile,
// picture-in-picture, fullscreen, and the existing episode nav/grid panel.
//
// Required element IDs kept identical to the old player so the rest of the
// watch page (watch-script1.ts) keeps working untouched:
//   #senshi-player-root, #sp-video, #sp-spinner (.hide toggles it),
//   #sp-error (.show toggles it) + #sp-err-msg, #sp-preplay (.hide toggles
//   it) + #sp-pp-btn, #sp-hls-badge.
import { h } from '../lib/helpers';

export interface PlayerProBodyParams {
  title: string;
  epNum: number;
...
export function playerProBody(p: PlayerProBodyParams): string {
  ...
  <div id="sp-video-area">
    ...
    <div id="sp-sc-overlay">
      <div>
        <div class="sp-sc-title">Keyboard Shortcuts</div>
        <div class="sp-sc-grid">
          ...
          <span class="sp-sc-key">C</span><span class="sp-sc-desc">Toggle subtitles</span>
          <span class="sp-sc-key">P</span><span class="sp-sc-desc">Picture-in-picture</span>
          ...
          <span class="sp-sc-key">0–9</span><span class="sp-sc-desc">Seek to %</span>
          <span class="sp-sc-key">?</span><span class="sp-sc-desc">This overlay</span>
        </div>
        <div style="text-align:center;margin-top:1rem">
          <button class="sp-sc-close" onclick="document.getElementById('sp-sc-overlay').classList.remove('show')">Close</button>
        </div>
      </div>
    </div>
    ...
    <div id="sp-topbar">
      <div>
        ...
        <div class="sp-top-ep">Episode ${epNum || 0}${currentEpTitle && currentEpTitle !== 'TBA' ? ' — ' + h(currentEpTitle) : ''}</div>
      </div>
    ...
    <div id="sp-controls">
      ...
      <div class="sp-btm">
        ...
        <button class="sp-btn sm" id="sp-cc-btn" data-tip="Subtitles (C)">
          <svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-10 7H8v-1H6v2h2v1H6v1h2v-1h2v-3zm7 0h-2v-1h-2v2h2v1h-2v1h2v-1h2v-3z"/></svg>
        </button>
        <button class="sp-btn sm" id="sp-pip-btn" data-tip="Picture-in-picture (P)">
          <svg viewBox="0 0 24 24"><path d="M19 7H5c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2zm0 12H5V9h14v10zM17 3H1v14h2V5h14V3z"/></svg>
        </button>
    ...
    <div class="sp-menu" id="sp-set-menu">
      ...
      <div class="sp-tab-panel sp-tab-hidden" id="sp-stab-subs">
        <div id="sp-sub-list">
          <div class="sp-sub-item active" id="sp-sub-disable">
        ...
        <div class="sp-sub-style-wrap" id="sp-sub-style-wrap">
          <div class="sp-sub-panel">
            ...
            <label><span>Position</span><input type="range" id="sp-sub-pos" min="4" max="40" value="8" step="1"></label>
            ...
            <label><span>Font</span>
              <select id="sp-sub-font">
                <option value="'Exo 2',Arial,sans-serif">Exo 2</option>
            ...
            <label><span>Color</span>
              <input type="color" id="sp-sub-color" value="#ffffff">
            </label>
          </div>
        </div>
      </div>
  ...
  <!-- Single floating tooltip for info strip -->
...
${epGridHtml}
      ...
      <div class="sp-psec" id="sp-ptab-stats">
        ...
        <div class="sp-buf-lbl">Buffer Health</div>
      ...
      <div class="sp-psec" id="sp-ptab-keys">
        <div class="sp-keys-grid">
          ...
          <div class="sp-key-row"><span class="sp-kbd">C</span><span class="sp-key-desc">Toggle subtitles</span></div>
          <div class="sp-key-row"><span class="sp-kbd">P</span><span class="sp-key-desc">Picture-in-picture</span></div>
